...
<div class="min-h-screen bg-gray-50">
    <div class="max-w-4xl mx-auto px-6 sm:px-8 lg:px-12 py-8">
        <!-- Back Button -->
        <div class="mb-6">
            <a href="{{ route('admin.categories') }}" class="inline-flex items-center text-blue-600 hover:text-blue-900 transition-colors duration-200">
        ...
        <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-8">
            ...
            <form action="{{ route('admin.update-category', $category) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="border-t border-gray-200 pt-6">
                    <div class="flex flex-col sm:flex-row gap-3">
                        ...
                        <a href="{{ route('admin.categories') }}" class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-700 hover:text-gray-900 font-semibold py-3 px-6 rounded-lg transition-colors duration-200 text-center">
                            Cancel
                        </a>
                    </div>
                </div>
        ...
        <div class="mt-6 bg-white shadow-sm rounded-lg border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                    ...
                    <span class="ml-2 text-gray-900">{{ $category->slug }}</span>
                </div>
                <div>
                    <span class="font-medium text-gray-500">Status:</span>
                    <span class="ml-2 inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $category->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        {{ $category->is_active ? 'Active' : 'Inactive' }}
                    </span>
                </div>
                <div>
                    <span class="font-medium text-gray-500">Products Count:</span>
                    <span class="ml-2 text-gray-900">{{ $category->products->count() ?? 0 }}</span>
                </div>
                <div>
