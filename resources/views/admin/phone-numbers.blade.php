                ...
                <div class="stat-card orders p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-gray-900">{{ number_format($stats['from_orders']) }}</p>
                ...
                <div class="stat-card manual p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-gray-900">{{ number_format($stats['manual']) }}</p>
                            ...
                            <tr class="table-row">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="w-10 h-10 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-semibold text-sm mr-4">
                                            {{ strtoupper(substr($phoneNumber->name ?: 'U', 0, 1)) }}
                                        </div>
                                        <div>
                                            <div class="text-sm font-medium text-gray-900">
                                                {{ $phoneNumber->name ?: 'Unknown Contact' }}
                                            </div>
                                            @if($phoneNumber->notes)
                                            <div class="text-xs text-gray-500">{{ Str::limit($phoneNumber->notes, 30) }}</div>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap hidden md:table-cell">
                                    <div class="text-sm text-gray-600">
                                        {{ $phoneNumber->address ? Str::limit($phoneNumber->address, 30) : 'No address' }}
                                    </div>
                                </td>
                                
                                <!-- Source -->
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="status-badge status-{{ $phoneNumber->source === 'order' ? 'order' : 'manual' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 hidden lg:table-cell">
                                    {{ $phoneNumber->collected_at ? $phoneNumber->collected_at->format('M j, Y') : $phoneNumber->created_at->format('M j, Y') }}
                                </td>
                                
                                <!-- Status -->
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="status-badge status-{{ $phoneNumber->is_active ? 'active' : 'inactive' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center space-x-3">
                                        <button class="text-blue-600 hover:text-blue-800 font-medium text-sm transition-colors duration-200"
                                                onclick="editPhoneNumber({{ $phoneNumber->id }})">
                                            Edit
                                        </button>
                ...
                <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
                    <div class="flex items-center justify-between">
                        <div class="text-sm text-gray-700">
                            Showing {{ $phoneNumbers->firstItem() }} to {{ $phoneNumbers->lastItem() }} of {{ $phoneNumbers->total() }} results
                        </div>
    ...
    <div id="addModal" class="fixed inset-0 modal-backdrop z-50 hidden" x-data="{ loading: false }">
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="modal-content max-w-lg w-full slide-up">
                ...
                <!-- Modal Body -->
                <form action="{{ route('admin.store-phone-number') }}" method="POST" class="p-8" x-on:submit="loading = true">
                    @csrf
                    <div class="space-y-6">
                        <div>
