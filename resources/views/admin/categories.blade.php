        ...
        <div class="max-w-full px-6 sm:px-8 lg:px-12 py-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                <div class="stats-card text-center p-8 rounded-2xl bg-blue-50 border border-blue-200 hover:shadow-lg transition-all duration-300">
                    ...
                    <div class="text-4xl font-bold mb-2 text-blue-600">{{ count($categories) }}</div>
                ...
                <div class="stats-card text-center p-8 rounded-2xl bg-green-50 border border-green-200 hover:shadow-lg transition-all duration-300">
                    ...
                    <div class="text-4xl font-bold mb-2 text-green-600">{{ $categories->sum('products_count') }}</div>
                ...
                <div class="stats-card text-center p-8 rounded-2xl bg-yellow-50 border border-yellow-200 hover:shadow-lg transition-all duration-300">
                    ...
                    <div class="text-4xl font-bold mb-2 text-yellow-600">{{ $categories->count() > 0 ? number_format($categories->sum('products_count') / $categories->count(), 1) : 0 }}</div>
                ...
                <div class="stats-card text-center p-8 rounded-2xl bg-green-50 border border-green-200 hover:shadow-lg transition-all duration-300">
                    ...
                    <div class="text-4xl font-bold mb-2 text-green-600">{{ $categories->where('is_active', true)->count() }}</div>
                ...
                <div id="categories-grid" class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                    ...
                    <div class="category-card bg-white rounded-2xl p-8 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-200 group">
                        <div class="flex items-start justify-between mb-6">
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-600 text-sm">{{ Str::limit($category->description ?: 'No description available.', 60) }}</p>
                            ...
                            <div class="flex flex-col items-end space-y-3">
                                <button onclick="toggleCategoryStatus({{ $category->id }})" 
                                    class="inline-flex items-center px-4 py-2 rounded-full text-sm font-bold transition-all duration-300 shadow-md hover:shadow-lg
                                        {{ $category->is_active ? 'bg-green-100 text-green-800 border border-green-200 hover:bg-green-200' : 'bg-red-100 text-red-800 border border-red-200 hover:bg-red-200' }}">
                        ...
                        <div class="flex space-x-3 pt-4 border-t border-gray-200">
                            <button onclick="viewCategory({{ $category->id }}, '{{ addslashes($category->name) }}', '{{ addslashes($category->description ?: 'No description available.') }}', {{ $category->is_active ? 'true' : 'false' }}, {{ $category->products->count() ?? 0 }})" 
                            ...
                            <form action="{{ route('admin.delete-category', $category->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this category? This action cannot be undone.');">
                                @csrf
                                @method('DELETE')
