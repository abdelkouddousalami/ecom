            ...
            <div class="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10">
                ...
                <div class="mb-4 lg:mb-6">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 lg:gap-3">
                        ...
                        <button onclick="filterByCategory('{{ $category->slug }}')" class="category-filter-btn bg-white hover:bg-gray-100 text-gray-700 hover:text-red-600 font-medium py-2 px-3 lg:px-4 rounded-lg transition-all duration-300 border-2 border-gray-200 hover:border-red-300" data-category="{{ $category->slug }}">
                            <span class="text-xs sm:text-sm font-semibold">{{ $category->name }}</span>
                        </button>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-8">
                ...
                <div class="product-card group bg-white rounded-xl border border-gray-200 hover:border-blue-300 transition-all duration-300 overflow-hidden cursor-pointer" 
                    ...
                    <div class="p-3 sm:p-4 lg:p-6">
                        ...
                        <!-- Description -->
                        <p class="text-gray-600 text-xs sm:text-sm mb-2 lg:mb-4 leading-relaxed">{{ Str::limit($product->description, 70) }}</p>
                        ...
                        <div class="flex items-center justify-between mb-3 lg:mb-4">
                            <div class="flex items-center">
                                @if($product->rating)
                                    <div class="flex items-center">
                                        @for($i = 1; $i <= 5; $i++)
                                            <svg class="w-3 h-3 sm:w-4 sm:h-4 {{ $i <= floor($product->rating) ? 'text-yellow-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                                            </svg>
                                        @endfor
                                        <span class="text-gray-600 text-xs sm:text-sm ml-1 sm:ml-2">({{$product->rating}})</span>
                                    </div>
                                @else
                        ...
                        <div class="flex items-center justify-between mb-3 lg:mb-5">
                            <div class="flex items-center flex-wrap">
                                <span class="text-lg sm:text-xl lg:text-2xl font-bold text-blue-600" style="font-family: 'Playfair Display', serif;">{{ number_format($product->price) }} DH</span>
                                @if($product->original_price && $product->original_price > $product->price)
                                    <span class="text-gray-500 line-through ml-1 sm:ml-2 text-xs sm:text-sm">{{ number_format($product->original_price) }} DH</span>
                                ...
                                @else
                                    ...
                                    @endphp
                                    <span class="text-gray-500 line-through ml-1 sm:ml-2 text-xs sm:text-sm">{{ number_format($sampleOriginalPrice) }} DH</span>
                                    <span class="bg-red-100 text-red-600 text-xs font-bold px-1 sm:px-2 py-1 rounded-full ml-1 sm:ml-2">
                                        -30%
                                    </span>
                                @endif
                        ...
                        <div class="space-y-2 lg:space-y-3">
                            <div class="flex gap-2 lg:gap-3">
                                ...
                                <button onclick="event.stopPropagation(); addToWishlist({{ $product->id }})" class="wishlist-btn-main bg-white border-2 border-gray-300 hover:border-red-400 text-gray-600 hover:text-red-500 font-medium py-2 lg:py-3 px-3 lg:px-4 rounded-lg transition-all duration-300 group cursor-pointer" data-product-id="{{ $product->id }}" title="Add to Favorites" style="font-family: 'Playfair Display', serif;">
            ...
            <div class="mt-8 lg:mt-16 flex justify-center">
                @if ($products->hasPages())
                    <nav class="flex items-center space-x-1 sm:space-x-2">
                        ...
                        @else
                            <a href="{{ $products->previousPageUrl() }}" class="px-2 sm:px-4 py-2 text-gray-600 hover:text-gray-900 transition-all duration-300 rounded-lg hover:bg-gray-100 font-medium text-xs sm:text-sm">Previous</a>
                        @endif

                        {{-- Pagination Elements --}}
                        @foreach ($products->getUrlRange(1, $products->lastPage()) as $page => $url)
                            @if ($page == $products->currentPage())
                                <span class="px-2 sm:px-4 py-2 bg-red-600 text-white rounded-lg shadow-lg font-semibold text-xs sm:text-sm">{{ $page }}</span>
                            @else
                                <a href="{{ $url }}" class="px-2 sm:px-4 py-2 text-gray-700 hover:text-gray-900 transition-all duration-300 rounded-lg hover:bg-gray-100 font-medium text-xs sm:text-sm">{{ $page }}</a>
                            @endif
                        @endforeach

                        {{-- Next Page Link --}}
                        @if ($products->hasMorePages())
                            <a href="{{ $products->nextPageUrl() }}" class="px-2 sm:px-4 py-2 text-gray-600 hover:text-gray-900 transition-all duration-300 rounded-lg hover:bg-gray-100 font-medium text-xs sm:text-sm">Next</a>
                        @else
    ...
    <script>
        ...
        function loadCartModal() {
            ...
            if (cart.length === 0) {
                ...
                cart.forEach(item => {
                    const product = @json($allProducts).find(p => p.id == item.id);
                    if (product) {
        ...
        function removeFromCartModal(productId) {
            let cart = JSON.parse(localStorage.getItem('cart') || '[]');
            const product = @json($allProducts).find(p => p.id == productId);
            cart = cart.filter(item => item.id != productId);
            localStorage.setItem('cart', JSON.stringify(cart));
            loadCartModal();
            updateCartCount();
